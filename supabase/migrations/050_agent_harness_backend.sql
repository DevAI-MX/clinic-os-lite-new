...
--   * 'openclaw' | 'hermes' = un gateway agéntico externo OpenAI-compat
--                  (corre SU propio loop de tools). Requiere agent_base_url.
--
...
ALTER TABLE public.ai_configs
  ...
  CHECK (agent_backend IN ('native', 'openclaw', 'hermes'));

COMMENT ON COLUMN public.ai_configs.agent_backend IS
  'Arnés agéntico: native (loops in-app, default) | openclaw | hermes (gateway externo OpenAI-compat).';
